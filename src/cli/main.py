...
from src.models.database import init_db, Podcast
        ...
        with Progress(
            ...
            if audio_only:
                ...
                cmd = [
                    ...
                    '-metadata', f'title={final_audio_name}',
                    '-metadata', 'artist=AI Podcast Creator',
                    '-metadata', f'album={config["character"]["name"]}',
                ...
                if avatar:
                    console.print(f"   🎭 With talking head avatar")
                if visualize:
                    console.print(f"   🎨 With {config.get('visualization', {}).get('style', 'waveform')} visualization")
...
def status():
    ...
    if gpu_manager.gpu_available:
        ...
        try:
            ...
            table.add_row("CUDA", "[green]✓[/green]", f"Version {cuda_version} (Compute {compute_cap[0]}.{compute_cap[1]})")
